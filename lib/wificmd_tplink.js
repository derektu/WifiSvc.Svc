...
var WifiCmd_TPLink = function(config) {
    ...
    self.getMacFilterList = function(cb) {
        self.doGetMacFilterList(function(err, macList) {
            if (err != null) {
                cb(err);
            }
            else {
                cb(null, {filterMode: true, macList:  getEnableMacList(macList)});
            }
        });
    };
    ...
    self.enableMac = function(mac, enable, cb) {
        self.doGetMacFilterList(function(err, macList) {
            if (err) {
                ...
                // look for mac index
                //
                var item = findMac(macList, macWithdash(mac));
                if (item == null) {
    ...
    /*
        ...
        TODO: 目前先不support (看是要抓guest mode, 還是調整2.4G的filter enable status)
     */
    self.setMacFilterMode = function(enable, callback) {
